...
function filterReg(){const q=(gv('regSearch')||'').toLowerCase();const pid=(gv('regPatientId')||'').toLowerCase();const inst=gv('regInstitusi');let data=D;if(q)data=data.filter(p=>p.nama.toLowerCase().includes(q)||p.nik.includes(q)||(p.perusahaan||'').toLowerCase().includes(q));if(pid)data=data.filter(p=>p.id.toLowerCase().includes(pid));document.getElementById('regTb').innerHTML=data.map(p=>{const stColor=p.statusKehadiran==='Hadir'?'#059669':'#F59E0B';const stText=p.statusKehadiran==='Hadir'?'Sudah Registrasi':'Belum Registrasi';return`<tr><td style="font-family:monospace;font-size:11px;color:var(--text-muted)">${p.id}</td><td><a href="javascript:void(0)" onclick="openRegDetail('${p.id}')" style="font-weight:600;color:#1a3a6b;text-decoration:none;cursor:pointer" title="Klik untuk buka form registrasi">${p.nama}</a><div style="font-size:10px;color:var(--text-muted)">${p.jenisKelamin||'-'} &bull; ${p.tanggalLahir||'-'}</div></td><td style="font-size:12px">${p.perusahaan?'<i class="fas fa-building" style="color:var(--text-muted);margin-right:4px;font-size:10px"></i>'+p.perusahaan:'-'}</td><td style="font-size:12px">${getPN(p.paketMCU)}</td><td><span style="color:${stColor};font-weight:600;font-size:11px">${stText}</span></td></tr>`;}).join('');}
function openRegForm(id){openRegDetail(id);}
...
function openAddPesertaFull(){const units=getAllUnits();const pakets=getAllPaketMCU();document.getElementById('fInstitusi').innerHTML='<option>-- Pilih --</option>'+units.map(u=>`<option value="${u.nama}">${u.nama}</option>`).join('');document.getElementById('fPak').innerHTML=pakets.map(p=>`<option value="${p.id}">${p.nama}</option>`).join('');const db=JSON.parse(localStorage.getItem('mcu_db')||'{}');const nextNum=(db.peserta?db.peserta.length:0)+1;document.getElementById('fPatientId').value='PA-'+String(nextNum).padStart(6,'0');openM('mAdd');}
function showAddPeserta(){openAddPesertaFull();}
// Document view for peserta
function openRegDoc(id){const p=getAllPeserta().find(x=>x.id===id);if(!p)return;const items=getPI(p.paketMCU);const checked=items.filter(it=>p.pemeriksaan&&p.pemeriksaan[it]&&p.pemeriksaan[it].status==='Checked').length;const noCase=p.noCase||String(Math.floor(Math.random()*9000)+1000);const tglMCU=p.tanggalDaftar?p.tanggalDaftar.split('T')[0]:new Date().toISOString().split('T')[0];const now=new Date();const generated=now.toLocaleDateString('id-ID')+', '+now.toLocaleTimeString('id-ID');const docNo=String(Math.floor(Math.random()*9000000)+1000000);const usia=p.tanggalLahir?Math.floor((now-new Date(p.tanggalLahir))/(365.25*24*60*60*1000))+' Tahun '+Math.floor(((now-new Date(p.tanggalLahir))%(365.25*24*60*60*1000))/(30.44*24*60*60*1000))+' Bulan':'-';const qrUrl='https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='+encodeURIComponent(p.id);const fotoHTML=p.foto?`<img src="${p.foto}" style="width:100%;height:100%;object-fit:cover;border-radius:10px">`:`<div style="width:100%;height:100%;background:#f4f6f9;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:9px;color:#a0aec0;text-align:center;border:1.5px dashed #ccd5e0;line-height:1.4;letter-spacing:.3px">TIDAK ADA<br>FOTO</div>`;
...
const iframeHTML=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;margin:0;padding:20px;background:#e8ecf4;color:#1a3a6b}.doc{max-width:720px;margin:0 auto;background:#fff;padding:24px 36px;box-shadow:0 4px 24px rgba(0,0,0,.08);line-height:1.3}table{width:100%;border-collapse:collapse}.lbl{font-size:11px;color:#718096;text-transform:uppercase;letter-spacing:.4px;margin:0 0 1px 0;padding:0;line-height:1.2}.val{font-size:15px;font-weight:700;color:#1a3a6b;margin:0;padding:0;line-height:1.4}.sec-hdr{background:#1a3a6b;color:#fff;padding:6px 12px;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:space-between;margin-top:10px;letter-spacing:.3px}@media print{body{padding:0;background:#fff}.doc{box-shadow:none;padding:16px 24px}}</style></head><body><div class="doc">
<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:0"><div><img src="mitra-logo.svg" style="height:72px;object-fit:contain"></div><img src="${qrUrl}" style="width:76px;height:76px;margin-bottom:-20px"></div>
<h2 style="font-size:16px;font-weight:900;color:#1a3a6b;margin:2px 0 2px">LEMBAR REGISTRASI MCU</h2>
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px"><span style="font-size:11px;color:#718096;font-family:monospace">Form No: ${docNo}</span><span style="font-family:monospace;font-size:12px;font-weight:700;color:#1a3a6b">${p.id}</span></div>
<div style="border-bottom:2.5px solid #1a3a6b;margin-bottom:12px"></div>
<div style="border-radius:8px;padding:6px 10px 6px 0;margin-bottom:4px;display:flex;gap:12px;align-items:flex-start"><div style="width:120px;height:130px;flex-shrink:0">${fotoHTML}</div><div style="flex:1"><div class="lbl">NAMA PESERTA</div><div style="font-size:16px;font-weight:800;color:#1a3a6b;margin:0 0 4px 0">${p.nama.toUpperCase()}</div><table><tr><td style="padding:1px 0 3px;width:48%;vertical-align:top"><div class="lbl">NIK (NOMOR KTP)</div><div class="val">${p.nik||'-'}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">JENIS KELAMIN / UMUR</div><div class="val">${(p.jenisKelamin||'LAKI-LAKI').toUpperCase()} / ${usia}</div></td></tr><tr><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. TELEPON / HP</div><div class="val">${p.telepon||'-'}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">PERUSAHAAN / INSTITUSI</div><div class="val">${p.perusahaan||'-'}</div></td></tr><tr><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. KARYAWAN</div><div class="val">${p.noKaryawan||'-'}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. CASE</div><div class="val">${noCase}</div></td></tr><tr><td colspan="2" style="padding:1px 0 3px;vertical-align:top"><div class="lbl">ALAMAT LENGKAP</div><div class="val" style="font-size:12px">${p.alamat||'-'}</div></td></tr></table><table style="margin-top:4px"><tr><td style="padding:3px 8px 3px 0;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">KODE PAKET MCU</div><div class="val" style="font-size:12px">${getPN(p.paketMCU)}</div></td><td style="padding:3px 8px;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">BATCH / CABANG MCU</div><div class="val" style="font-size:12px;color:#2563eb">${p.institusi||'Mitra Keluarga'}</div></td><td style="padding:3px 8px;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">STATUS & JADWAL</div><div class="val" style="font-size:11px">${p.statusKehadiran==='Hadir'?'SUDAH REGISTRASI':'BELUM REGISTRASI'}<br>(${tglMCU})</div></td><td style="padding:3px 8px;width:25%;vertical-align:top"><div class="lbl">REGISTRASI TIMESTAMP</div><div class="val" style="font-size:10px">${p.waktuRegistrasi?new Date(p.waktuRegistrasi).toLocaleDateString('id-ID')+',<br>'+new Date(p.waktuRegistrasi).toLocaleTimeString('id-ID'):'-'}</div></td></tr></table></div></div>
<div class="sec-hdr"><span>DAFTAR PEMERIKSAAN KESEHATAN (MCU)</span><div><span style="padding:2px 8px;border:1px solid rgba(255,255,255,.4);border-radius:3px;font-size:9px;margin-right:6px">${getPN(p.paketMCU)}</span><span style="font-size:9px">${checked}/${items.length} Selesai</span></div></div>
<table style="font-size:11px;border:1.5px solid #e0e6ef"><thead><tr style="background:#f4f6fa"><th style="padding:4px 4px;border:1px solid #e0e6ef;text-align:center;width:30px;font-weight:700;font-size:10px">No.</th><th style="padding:4px 4px;border:1px solid #e0e6ef;font-weight:700;font-size:10px">Item Pemeriksaan (Checklist)</th><th style="padding:4px 4px;border:1px solid #e0e6ef;text-align:center;width:44px;font-weight:700;font-size:10px">Status Check</th><th style="padding:4px 4px;border:1px solid #e0e6ef;width:80px;font-weight:700;font-size:10px">Paraf Petugas</th><th style="padding:4px 4px;border:1px solid #e0e6ef;width:80px;font-weight:700;font-size:10px">Tgl / Jam Check</th></tr></thead><tbody>${items.map((it,i)=>{const d=p.pemeriksaan&&p.pemeriksaan[it]?p.pemeriksaan[it]:{};const isDone=d.status==='Checked';return`<tr><td style="padding:3px 4px;border:1px solid #e0e6ef;text-align:center">${i+1}</td><td style="padding:3px 4px;border:1px solid #e0e6ef">${it}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;text-align:center;font-size:12px">${isDone?'☑':'☐'}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;font-size:8px;color:#718096">${isDone?(d.petugas||''):''}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;font-size:8px;color:#718096">${isDone?tglMCU:''}</td></tr>`;}).join('')}</tbody></table>
<div style="display:flex;gap:20px;margin-top:12px;align-items:flex-start"><div style="border:1.5px solid #e0e6ef;border-radius:6px;padding:8px 12px;min-height:50px;width:45%"><div style="font-size:9px;font-weight:700;color:#718096;text-transform:uppercase;margin-bottom:2px">CATATAN PENDAFTARAN KHUSUS:</div><div style="font-size:11px;color:#1a3a6b">${p.catatanReg||'-'}</div></div><div style="flex:1;display:flex;justify-content:flex-end;gap:40px;font-size:9px;padding-top:4px"><div style="text-align:center;min-width:130px"><div style="border-top:1px solid #1a3a6b;margin-top:44px;padding-top:3px">Tanda Tangan Peserta</div></div><div style="text-align:center;min-width:130px"><div style="font-size:10px;font-weight:700;color:#1a3a6b;margin-bottom:2px">${(()=>{try{return JSON.parse(localStorage.getItem('mcu_user')||'{}').nama||'Admin';}catch(e){return 'Admin';}})()}</div><div style="border-top:1px solid #1a3a6b;margin-top:34px;padding-top:3px">Petugas Pendaftaran</div></div></div></div>
<div style="margin-top:12px;font-size:8px;color:#a0aec0;display:flex;justify-content:space-between"><span>MCU System — Mitra Keluarga</span><span>Dicetak: ${generated}</span></div>
</div></body></html>`;
...
// DATA PESERTA MEDICAL CHECK UP - untuk Daftar Peserta / Kehadiran
...
const iframeHTML=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;margin:0;padding:20px;background:#e8ecf4;color:#1a3a6b}.doc{max-width:720px;margin:0 auto;background:#fff;padding:24px 36px;box-shadow:0 4px 24px rgba(0,0,0,.08);line-height:1.3}table{width:100%;border-collapse:collapse}.lbl{font-size:11px;color:#718096;text-transform:uppercase;letter-spacing:.4px;margin:0 0 1px 0;padding:0;line-height:1.2}.val{font-size:15px;font-weight:700;color:#1a3a6b;margin:0;padding:0;line-height:1.4}.sec-hdr{background:#1a3a6b;color:#fff;padding:6px 12px;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:space-between;margin-top:10px;letter-spacing:.3px}@media print{body{padding:0;background:#fff}.doc{box-shadow:none;padding:16px 24px}}</style></head><body><div class="doc">
<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:0"><div><img src="mitra-logo.svg" style="height:72px;object-fit:contain"></div><img src="${qrUrl}" style="width:76px;height:76px;margin-bottom:-20px"></div>
<h2 style="font-size:16px;font-weight:900;color:#1a3a6b;margin:2px 0 2px">DATA PESERTA MEDICAL CHECK UP</h2>
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px"><span style="font-size:11px;color:#718096;font-family:monospace">Doc No: ${docNo} &bull; Tanggal: ${tglMCU}</span><span style="font-family:monospace;font-size:12px;font-weight:700;color:#1a3a6b">${p.id}</span></div>
<div style="border-bottom:2.5px solid #1a3a6b;margin-bottom:12px"></div>
<div style="padding:6px 10px 6px 0;margin-bottom:4px;display:flex;gap:12px;align-items:flex-start"><div style="width:120px;height:130px;flex-shrink:0">${fotoHTML}</div><div style="flex:1"><div class="lbl">NAMA PESERTA</div><div style="font-size:16px;font-weight:800;color:#1a3a6b;margin:0 0 4px 0">${p.nama.toUpperCase()}</div><table><tr><td style="padding:1px 0 3px;width:48%;vertical-align:top"><div class="lbl">PATIENT ID</div><div class="val">${p.id}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">JENIS KELAMIN / UMUR</div><div class="val">${(p.jenisKelamin||'LAKI-LAKI').toUpperCase()} / ${usia}</div></td></tr><tr><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. TELEPON / HP</div><div class="val">${p.telepon||'-'}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">PERUSAHAAN / INSTITUSI</div><div class="val">${p.perusahaan||'-'}</div></td></tr><tr><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. KARYAWAN</div><div class="val">${p.noKaryawan||'-'}</div></td><td style="padding:1px 0 3px;vertical-align:top"><div class="lbl">NO. CASE</div><div class="val">${noCase}</div></td></tr></table><table style="margin-top:4px"><tr><td style="padding:3px 8px 3px 0;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">PAKET MCU</div><div class="val" style="font-size:12px">${getPN(p.paketMCU)}</div></td><td style="padding:3px 8px;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">BATCH / CABANG</div><div class="val" style="font-size:12px;color:#2563eb">${p.institusi||'Mitra Keluarga'}</div></td><td style="padding:3px 8px;width:25%;border-right:1px solid #e0e6ef;vertical-align:top"><div class="lbl">STATUS</div><div class="val" style="font-size:12px">${p.statusKehadiran==='Hadir'?'SUDAH REGISTRASI':'BELUM REGISTRASI'}</div></td><td style="padding:3px 8px;width:25%;vertical-align:top"><div class="lbl">TANGGAL MCU</div><div class="val" style="font-size:12px">${tglMCU}</div></td></tr></table></div></div>
<div class="sec-hdr"><span>DAFTAR ITEM PEMERIKSAAN</span><div><span style="padding:2px 8px;border:1px solid rgba(255,255,255,.4);border-radius:3px;font-size:9px;margin-right:6px">${getPN(p.paketMCU)}</span><span style="font-size:9px">${checked}/${items.length} Selesai</span></div></div>
<table style="font-size:11px;border:1.5px solid #e0e6ef"><thead><tr style="background:#f4f6fa"><th style="padding:4px 4px;border:1px solid #e0e6ef;text-align:center;width:30px;font-weight:700;font-size:10px">No.</th><th style="padding:4px 4px;border:1px solid #e0e6ef;font-weight:700;font-size:10px">Item Pemeriksaan</th><th style="padding:4px 4px;border:1px solid #e0e6ef;text-align:center;width:44px;font-weight:700;font-size:10px">Status Check</th><th style="padding:4px 4px;border:1px solid #e0e6ef;width:70px;font-weight:700;font-size:10px">Petugas</th><th style="padding:4px 4px;border:1px solid #e0e6ef;width:70px;font-weight:700;font-size:10px">Tgl / Jam</th></tr></thead><tbody>${items.map((it,i)=>{const d=p.pemeriksaan&&p.pemeriksaan[it]?p.pemeriksaan[it]:{};const isDone=d.status==='Checked';return`<tr><td style="padding:3px 4px;border:1px solid #e0e6ef;text-align:center">${i+1}</td><td style="padding:3px 4px;border:1px solid #e0e6ef">${it}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;text-align:center;font-size:12px">${isDone?'☑':'☐'}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;font-size:8px;color:#718096">${isDone?(d.petugas||''):''}</td><td style="padding:3px 4px;border:1px solid #e0e6ef;font-size:8px;color:#718096">${isDone?tglMCU:''}</td></tr>`;}).join('')}</tbody></table>
<div style="display:flex;justify-content:space-between;margin-top:14px;font-size:9px"><div style="text-align:center;min-width:130px"><div style="border-top:1px solid #1a3a6b;margin-top:44px;padding-top:3px">Tanda Tangan Peserta</div></div><div style="text-align:center;min-width:130px"><div style="border-top:1px solid #1a3a6b;margin-top:44px;padding-top:3px">Petugas MCU</div></div><div style="text-align:center;min-width:130px"><div style="border-top:1px solid #1a3a6b;margin-top:44px;padding-top:3px">Koordinator MCU</div></div></div>
<div style="margin-top:12px;font-size:8px;color:#a0aec0;display:flex;justify-content:space-between"><span>MCU System — Mitra Keluarga</span><span>Dicetak: ${generated}</span></div>
</div></body></html>`;
overlay.innerHTML=`<div style="padding:12px 24px;display:flex;justify-content:flex-end;gap:8px;background:#fff;border-bottom:1px solid #e0e6ef;flex-shrink:0"><button onclick="document.getElementById('docOverlay').style.display='none'" style="padding:8px 16px;background:#fff;border:1px solid #ddd;border-radius:6px;font-size:12px;cursor:pointer">Kembali</button><button onclick="document.getElementById('docOverlay').style.display='none'" style="padding:8px 16px;background:#fff;border:1px solid #ddd;border-radius:6px;font-size:12px;cursor:pointer">Tutup</button><button onclick="document.getElementById('docIframe').contentWindow.print()" style="padding:8px 16px;background:#1a3a6b;color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:700;cursor:pointer"><i class="fas fa-print"></i> Cetak Dokumen</button></div><iframe id="docIframe" style="flex:1;border:none;width:100%"></iframe>`;
const iframe2=overlay.querySelector('#docIframe');iframe2.contentDocument.open();iframe2.contentDocument.write(iframeHTML);iframe2.contentDocument.close();}
...
// openPemDetail is defined in pem-detail.js (in-page view)
// openPem and savePem are no longer used (replaced by pem-detail.js savePemSection)
...
document.addEventListener('DOMContentLoaded',()=>{
  // Force reset peserta data (clean slate)
  if(!localStorage.getItem('mcu_peserta_reset_v4')){
    localStorage.removeItem('mcu_db');
    localStorage.setItem('mcu_peserta_reset_v4','1');
  }
  checkLogin();const sb=document.querySelector('.sb');if(sb)sb.style.display='none';const mn=document.querySelector('.mn');if(mn){mn.style.marginLeft='0';mn.style.background='transparent';}initDefaultUsers();initDefaultLevels();initDefaultUnits();initDefaultPaketMCU();initDefaultStatusAlur();
});
